                      ...
                      <div class="row">
                        <div class="col-md-10">
                            <div class="card-icon">
                              ...
                              <h4 class="card-title">Total de mes Achats: <strong>{{$total}} </strong> </h4>
                        </div>
                        ...
                        <table class="table table-striped">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>
                                <img src="{{asset('storage/images/'.$achat->commandes->produits->image)}}" width="100" height="100" alt="">
                              </td>
                              <td class="text-center">{{$achat->commandes->produits->nom}}</td>
                              <td class="text-center">{{$achat->quantite}}</td>
                              <td class="text-center">{{$achat->commandes->prixVente}}</td>
                              <td class="text-center">{{$achat->created_at}}</td>
                              <td class="text-center">{{$achat->prixSortie}}</td>
                              <td class="td-actions text-right d-flex align-item-center justify-content-end">
                                <a href="{{route('produits.show',$achat->id)}}" rel="tooltip" class="btn btn-info btn-round mx-1">
                                    <i class="material-icons">person</i>
                                </a>
                                <form action="{{route('commande.destroy',$achat->id)}}" method="post" >
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger btn-round" rel="tooltip" >
                                    <i class="material-icons">close</i>
